...
var Commands = {
    ...
    '/msg': {
        ...
        Adjust: function (data, socket) {
            ...
            var receiverUsername = data.message.substring(0, data.message.indexOf(' ')).trim();
            if (receiverUsername === '')
                return;
            data.message = data.message.substring(receiverUsername.length, data.message.length).trim();
            var receiverID, receiverData = {
    ...
    '/r': {
        ...
        Adjust: function (data, socket) {
            ...
            var receiverID = users[socket.id].lastMessenger;
            if (typeof receiverID === "undefined") {
                data.type = 'command';
...
io.on('connection', function (socket) {
    ...
    socket.on('chat-userMessage', function (data) {
        data['type'] = 'user-chat';
    ...
    socket.on('disconnect', function () {
        ...
        var userIndex = usersInChat.indexOf(users[socket.id].username);
